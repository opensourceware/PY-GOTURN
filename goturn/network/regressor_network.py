...
class Model(nn.Module):
	def __init__(self):
		...
		for param in self.fc9.parameters():
			param.requires_grad = True
	...
	def forward(self, img, ref, hidden):
		...
		img_output = F.max_pool2d(F.relu(self.conv5(img_output)), 3, 2)
		...
		ref_output = F.max_pool2d(F.relu(self.conv5(ref_output)), 3, 2)

		output = torch.cat((img_output.view(self.seq_len, 9216), ref_output.view(self.seq_len, 9216)), 1)

		output = F.dropout(F.relu(self.fc6(output)))
		output = F.dropout(F.relu(self.fc7(output)))
		output = F.dropout(F.relu(self.fc7b(output)))
		output = F.dropout(F.relu(self.fc8(output)))
		output, hidden = self.lstm(torch.unsqueeze(output, 1))
	...
	def fit(self, input_dir, grad=True):
		...
		for file in data_files:
			...
			ground_truth = Variable(torch.from_numpy(ground_truth[1:]).type(torch.FloatTensor), requires_grad=False)
			hidden_0 = self.initHidden()
			pred = self.forward(inp, target, hidden_0)
			loss = self.loss(torch.squeeze(pred, 1), ground_truth)
			if grad:
	...
	def train(self, input_dir, epochs=50):
		vars = []
		for param in self.parameters():
			if param.requires_grad == True:
				vars.append(param)
		self.optimizer = torch.optim.Adam(vars, lr=0.001)
		prev_loss = np.inf
		for iter in range(epochs):
			self.fit(input_dir)
			train_loss = self.fit(input_dir, False)
			print('Loss epoch %i is %f' %(iter, train_loss))
			if float(train_loss.data)<prev_loss:
				with open('tracker.pt', 'w') as f:
					torch.save(self.state_dict(), f)
			prev_loss = float(train_loss.data)
...
def main():
	model = Model()
	model.load_state_dict(torch.load("/home/manpreetkaur/FABMAP/tracking/PY-GOTURN/goturn/network/tracker.pt"))
	input_dir = "/mnt/nfs/scratch1/manpreetkaur/tracking/data/"
